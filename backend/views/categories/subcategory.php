...
?>
 ...
 <?php 
       ...
       <div class="d-flex justify-content-between  flex-column flex-sm-row">
            <div>
                <h4 class="m-b25 blueTxtClr p-t10 p-b10"><?php echo $category->name; ?></h4>
            </div>
            <div class="">
              <button class='btn btn-primary align-text-top border-0 m-b10'>
                    <?=Html::a('<i class="fa fa-plus p-r10"></i> '.Yii::t('app','Add Category'),['categories/add','id'=>$_GET['id']]); ?> &nbsp;&nbsp;&nbsp;
                </button> 
            </div>
        </div>
              ...
              [
                ...
                'value' => function ($model) { 
                    if(yii::$app->Myclass->SubCategoryCount($model->categoryId)!=0) {
                          return Html::a(yii::$app->Myclass->SubCategoryCount($model->categoryId).' ', ['categories/sub_subcategory', 'id' => $model->categoryId],['class'=>'btn btn-primary']);
                      }
                      else
                      {
                          return Html::a('0', ['categories/sub_subcategory', 'id' => $model->categoryId],['class'=>'btn btn-primary']);
                      }
                }
            ],
       ['class' => 'yii\grid\ActionColumn',
                    ...
                    'buttons' => [
                        'update' => function ($url, $model) {
                              $url = Url::to(['categories/updatesubcategory', 'id' => $model->categoryId,'cat'=>$_GET['id']]); 
