								...
								<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding margin-top-20">
									<div>
										...
										<div class="adtab-message primary-txt-color status-txt">
											...
											<?php } else { ?>
												<?=yii::t('app', 'Pending');?>
												<?php $stripe_key = $stripesettings['stripePublicKey'];?>
												<input type="hidden" value="<?php echo $stripe_key; ?>" id="stripekey" >
												<button type="button" id="StripePayBtn" class="post-btn btnUpdate btn" >Continue with Payment</button>
											<?php }?>
										</div>
									</div>
								</div>
...
var currency ='<?php echo $model['currency']; ?>';
var amount ='<?php echo $model['totalCost']; ?>';
$.ajax({
url: '<?=Yii::$app->getUrlManager()->getBaseUrl()?>/site/bannerstripesessioncreation/',
type: "POST",
dataType:"json",
data:{"bannerid": bannerid,"currency":currency,"amount": amount,},
