						...
						<div id="recent-activity" class="profile-tab-content tab-pane fade in active col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding <?php echo $fempty_tap; ?>">
							<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding">
								<ul class="recent-activities-tab nav nav-tabs">
								  ...
								  <li class="active" id="following_active">
...
if($count > 0) {
foreach($followinglist as $followinglistt):
	$followersdet = yii::$app->Myclass->getUserDetailss($followinglistt->follow_userId);
	$productdet = yii::$app->Myclass->getUserProductDetails($followinglistt->follow_userId, 4);
...
<div class="profile-listing-product product-padding col-xs-12 col-sm-6 col-md-4 col-lg-4" style="">
	<div class="followers-cnt col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding">
		<div class="followers-prof-pic-cnt col-xs-12 col-sm-5 col-md-5 col-lg-5 no-hor-padding">
			<a href="<?php echo Yii::$app->urlManager->createAbsoluteUrl(['user/profiles','id'=>yii::$app->Myclass->safe_b64encode($followersdet['userId'].'-'.rand(0,999))]); ?>"><div class="followers-prof-pic-1" style="background-image: url('<?php echo Yii::$app->urlManager->createAbsoluteUrl($img); ?>');"></div></a>
		</div>
...
<?php } ?>											
									...
									<?php if(count($followinglist) >= 15) {
										 ?>
									<div class="load-more-cnt col-xs-12 col-sm-12 col-md-12 col-lg-12 no-hor-padding">
									<div class="following-loader classified-loader" style="width: 60px;"><div class="cssload-loader"></div></div>
									<a class="loadmorenow load">
									<div class="load-more-icon" onclick="load_more_followings('<?php echo yii::$app->Myclass->safe_b64encode($user->userId.'-'.rand(0,999)) ?>')"></div>
...
function load_more_followings(id)
{
	$.ajax({
        url: '<?=Yii::$app->getUrlManager()->getBaseUrl()?>/user/moreloadfollowing/',
        type: "POST",
        dataType : "html",
        data: {
        ...
        beforeSend: function(data){
			$(".following-loader").show();
			$(".load").hide();
				},
        success: function (res) {
           $(".following-loader").hide();
				$(".load").show();
		        var output = res.trim();
						if (output) {
							offset = offset + limit;
					$("#following").append(output);
				            ...
				            $(".load").html(yii.t('app',"No More Followings"));
						}
        },
    });
}
